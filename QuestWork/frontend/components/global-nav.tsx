...
export function GlobalNav({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
  ...
  const makeEnter = (setter: (v: boolean) => void, timerRef: React.MutableRefObject<NodeJS.Timeout | null>) => () => {
    if (timerRef.current) { clearTimeout(timerRef.current); timerRef.current = null }
    setter(true)
  }

  const makeLeave = (setter: (v: boolean) => void, timerRef: React.MutableRefObject<NodeJS.Timeout | null>) => () => {
    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background">
      <nav className="mx-auto flex max-w-content items-center justify-between px-4 py-4 sm:px-6">
        ...
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="hidden font-bold text-foreground sm:inline">QuestWork</span>
        </Link>
                  ...
                  {QUEST_CATEGORIES.map((category) => (
                    <Link
                      ...
                      className={`block rounded-lg p-3 transition-colors ${
                        category.featured ? 'bg-primary-light' : 'hover:bg-surface'
                      }`}
                    >
                      <div className={`text-sm font-semibold ${category.featured ? 'text-primary' : 'text-foreground'}`}>
                        {category.title}
                      </div>
                      <div className="mt-1 text-xs text-foreground-muted">{category.description}</div>
                    </Link>
                  ))}
                </div>
              </div>
